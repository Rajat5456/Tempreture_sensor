...
static void One_wire_data_pin_Init(void);
...
void OWRead_data(void)
{
        ...
        {
        	RH_Data[loop] = result[loop];
        }
        for(loop = 16; loop < 32; loop++)
         {
                	T_Data[i] = result[loop];
                	i++;
          }
        ...
        HAL_UART_Transmit (&huart2,RH_Data, sizeof(RH_Data), 10);
        HAL_UART_Transmit (&huart2,"\n", sizeof("\n"), 10);
        HAL_UART_Transmit (&huart2,T_Data, sizeof(T_Data), 10);
        HAL_UART_Transmit (&huart2,"\n", sizeof("\n"), 10);
        HAL_UART_Transmit (&huart2,crc, sizeof(crc), 10);
        HAL_UART_Transmit (&huart2,"\n", sizeof("\n"), 10);
}
